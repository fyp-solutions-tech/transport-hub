...
export default function BookPage() {
  ...
  useEffect(() => {
    ...
    watchId.current = navigator.geolocation.watchPosition(
      (position) => {
        ...
        // Update live location (the blue dot), NOT the pickup directly
      ...
      { 
        ...
        timeout: 30000 // 30s timeout for better precision in rural areas like Narowal
      }
    );
  ...
  useEffect(() => {
    if (!liveLocation || liveLocation.address !== "Detecting address...")
      return;
  ...
  useEffect(() => {
    ...
    if (
      ...
      (places as any).PlaceAutocompleteElement
    ) {
      ...
      // Initialize with existing pickup address if present
      if (pickup?.address) {
        pickupAutocompleteRef.current.value = pickup.address;
      }

      pickupAutocompleteRef.current.addEventListener(
        "gmp-placeselect",
        (e: any) => {
          const place = e.place;
          if (place.location) {
    ...
    if (
      ...
      // Initialize with existing dropoff address if present
      if (dropoff?.address) {
        dropoffAutocompleteRef.current.value = dropoff.address;
      }

      dropoffAutocompleteRef.current.addEventListener(
        "gmp-placeselect",
        (e: any) => {
          const place = e.place;
          if (place.location) {
  ...
  const handleManualGeocode = async (type: "pickup" | "dropoff") => {
    ...
    const value = ref.current?.value; // Get text from web component if possible
    if (!value || (type === "pickup" ? pickup : dropoff)) return;

    try {
      const geocoder = new google.maps.Geocoder();
      const response = await geocoder.geocode({ address: value, componentRestrictions: { country: "pk" } });
      if (response.results[0]) {
        const { lat, lng } = response.results[0].geometry.location;
        const loc = { lat: lat(), lng: lng(), address: response.results[0].formatted_address };
  ...
  // ── Computed fare for selected vehicle ───w───────────
  const fare = useMemo(() => {
  ...
  const vehicleIcon = (icon: string) => {
    switch (icon) {
      ...
      case "electric":
        return <MdElectricCar className="text-success" />;
      case "moto":
        return <FaMotorcycle className="text-warning" />;
      default:
        return <MdDirectionsCar />;
    }
  };
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="card bg-base-100 border border-base-200 shadow-sm relative overflow-visible">
        <div className="card-body gap-5 p-4">
          ...
          <div className="form-control">
            ...
            <div className="flex items-center gap-2 h-[52px]">
              <div className="flex items-center gap-3 bg-base-200/50 hover:bg-base-200 focus-within:bg-base-200 rounded-xl px-3 border border-base-200 relative grow h-full">
                ...
                <div
                  ref={pickupRef}
                  className="grow w-full relative h-[24px] *:absolute *:-top-3 *:bg-transparent! *:w-full"
                  style={pacStyles}
                />
              </div>
              {/* GPS Refresh Button */}
              <button
                ...
                disabled={isLocating}
                className="btn btn-square btn-ghost border border-base-200 rounded-xl h-full w-[52px] tooltip tooltip-bottom"
                data-tip="Refresh GPS"
              >
                {isLocating ? (
                  <span className="loading loading-spinner loading-xs" />
                ) : (
                  <MdGpsFixed className="text-xl text-success" />
                )}
              </button>
            </div>
            ...
            {liveLocation?.address &&
              liveLocation.address !== "Detecting address..." && (
                <div className="flex items-center justify-between mt-1.5 pl-1">
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      setPickup(liveLocation);
                      if (pickupAutocompleteRef.current) {
                        pickupAutocompleteRef.current.value = liveLocation.address;
                      }
                      if (map) map.panTo({ lat: liveLocation.lat, lng: liveLocation.lng });
                      toast.success("Pickup set to live location");
                    }}
                    className="text-[10px] font-bold text-primary hover:underline uppercase tracking-tight shrink-0"
                  >
                    Use as Pickup
                  </button>
                </div>
          ...
          <div className="form-control">
            ...
            <div className="flex items-center gap-3 bg-base-200/50 hover:bg-base-200 rounded-xl px-3 border border-base-200 h-[52px]">
              <div className="bg-error/10 p-2 rounded-lg">
                <MdLocationOn className="text-error text-xl" />
              </div>
              <div
                ref={dropoffRef}
                onBlur={() => handleManualGeocode("dropoff")}
                className="grow w-full relative h-[24px] *:absolute *:-top-3 *:bg-transparent! *:w-full"
                style={pacStyles}
              />
            </div>
        ...
        {!dropoff && (
          <div className="text-center py-10 bg-base-200/30 rounded-2xl border border-dashed border-base-300">
            <p className="text-base-content/40 text-sm">
              Select locations to see fare estimates
            </p>
          </div>
        )}
          ...
          <div className="grid grid-cols-1 gap-2.5">
            {VEHICLES.map((v) => {
              ...
              return (
                ...
                >
                  <div className={`card border-2 transition-all duration-300 shadow-sm ${
                    isSelected 
                    ...
                    <div className="card-body flex-row items-center gap-4 py-3.5 px-4">
                      <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-3xl transition-colors duration-300 ${
                        isSelected ? "bg-primary text-white" : "bg-base-200 text-base-content/70 group-hover:bg-base-300"
                      }`}>
                        {vehicleIcon(v.icon)}
                      </div>
      ...
      >
        {fare
          ? `Confirm ${selectedVehicle === "moto" ? "Motorbike" : selectedVehicle === "comfort" ? "Comfort" : "Economy"} — Rs ${fare.total}`
          : "Confirm Booking"}
